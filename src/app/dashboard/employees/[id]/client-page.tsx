      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        <Card className="lg:col-span-1">
            ...
            <CardContent className="space-y-4">
                ...
                <DetailItem icon={<Calendar className="h-5 w-5"/>} label="Date of Birth" value={isClient ? formatDate(employee.dateOfBirth) : 'Loading...'} />
        ...
        <Card className="lg:col-span-1">
            ...
            <CardContent className="space-y-4">
                ...
                <DetailItem icon={<Calendar className="h-5 w-5"/>} label="Mess Entry Date" value={isClient ? formatDate(employee.messEntryDate) : 'Loading...'} />
        ...
        <Card className="lg:col-span-3">
             ...
             <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                 <DetailItem icon={<Calendar className="h-5 w-5"/>} label="Contract Start Date" value={isClient ? formatDate(employee.contractStartDate) : 'Loading...'} />
                 <div className="flex items-start gap-4">
                 ...
                 {(employee.employeeStatus === 'resign' || employee.employeeStatus === 'phk') ? (
                    <DetailItem icon={<UserX className="h-5 w-5"/>} label="Tanggal Efektif Resign/PHK" value={isClient ? formatDate(employee.contractEndDate) : 'Loading...'} />
                 ) : (
                    <DetailItem icon={<Calendar className="h-5 w-5"/>} label="Contract End Date" value={isClient ? formatDate(employee.contractEndDate) : 'Loading...'} />
                 )}
            </CardContent>
        </Card>
                    ...
                    <TableBody>
                        {employee.leaveHistory && employee.leaveHistory.length > 0 ? (
                            employee.leaveHistory.map((req) => (
                                <TableRow key={req.id}>
                                    <TableCell>{isClient ? formatDate(req.startDate) : 'Loading...'}</TableCell>
                                    <TableCell>{isClient ? formatDate(req.endDate) : 'Loading...'}</TableCell>
