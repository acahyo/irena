...
import { LogIn, AlertCircle, Contact, Building } from 'lucide-react';
...
export default function PjLoginPageClient({ settings }: { settings: AppSettings }) {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const result = await authenticateUser({ email, password }, 'pj');

        if (result.success) {
            router.push('/pj-dashboard');
        } else {
             ...
             <div className="mt-4 text-center text-sm">
              <Link href="/" className="underline text-muted-foreground hover:text-primary">
                 <div className="flex items-center justify-center gap-1">
                    <Building className="h-4 w-4" /> {T.notPj}
                </div>
              </Link>
            </div>
          </CardContent>
