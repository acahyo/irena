...
import { LogIn, AlertCircle, Car, Building } from 'lucide-react';
...
export default function DriverLoginPageClient({ settings }: { settings: AppSettings }) {
    ...
    const T = useMemo(() => ({
        welcome: lang === 'id' ? 'Selamat datang! Silakan masuk ke akun Driver Anda.' : 'Welcome! Please sign in to your Driver account.',
        ...
        notDriver: lang === 'id' ? 'Bukan driver? Masuk sebagai Admin' : 'Not a driver? Sign in as Admin',
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const result = await authenticateUser({ email, password }, 'driver');
             ...
             <div className="mt-4 text-center text-sm">
              <Link href="/" className="underline text-muted-foreground hover:text-primary">
                 <div className="flex items-center justify-center gap-1">
                    <Building className="h-4 w-4" /> {T.notDriver}
                </div>
              </Link>
            </div>
          </CardContent>
